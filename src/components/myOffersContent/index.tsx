...
export function MyOffersContent({ data }: MyOffersContentProps) {

    useGSAP(() => {
        const tl = gsap.timeline({defaults: { stagger:0.4, duration:0.6, ease: 'power1.out' }})
    
        tl.fromTo('.subtitle', { opacity:0, x:30 }, { opacity:1, x:0 });
        tl.fromTo('.btn', { opacity:0 }, { opacity:1, ease:'bounce' });
        tl.fromTo('.filterTitle', { opacity:0 }, { opacity:1 });
        tl.fromTo('.filterSearch', { opacity:0, y:30 }, { opacity:1, y:0 });
        tl.fromTo('.filterSelect', { opacity:0, y:30 }, { opacity:1, y:0 });
        tl.fromTo('.tDataResponsive',{ opacity:0, y:30 }, { opacity:1, y:0 });
        tl.fromTo('.tableContent', { opacity:0, scale:0 }, { opacity:1, scale:1 });
        tl.fromTo('.tHead', { opacity:0, y:30 }, { opacity:1, y:0 });
        tl.fromTo('.tData', { opacity:0, y:30 }, { opacity:1, y:0 });
    }, []);
        ...
        <>
            <div className="py-8 container">
                <div className="flex justify-between items-center px-4 lg:px-0">
                    <h2 className="subtitle text-[#6B7280] text-xl font-medium">Minhas ofertas</h2>
                    <Link href="/" className="btn font-medium p-3 max-w-40 w-full rounded-full bg-[#1E90FF] text-white flex justify-center items-center gap-3 cursor-pointer transition-all duration-500 hover:brightness-90">
                ...
                <div className="mt-8 border border-transparent lg:border-gray-300 rounded-lg px-4 lg:px-0">
                    <div className="pb-3 lg:p-3 flex flex-col lg:flex-row gap-4 lg:gap-0 items-center">
                        <h3 className="filterTitle text-lg text-[#6B7280] hidden lg:block">Todas as ofertas</h3>
                        <div className="grid grid-cols-3 lg:flex gap-4 w-full lg:w-auto ml-0 lg:ml-auto">
                            <div className="filterSearch col-span-2 lg:w-60 p-2 border border-gray-300 rounded-full flex gap-3 items-center">
                                <input
                                    type="text"
                            ...
                            <div className="filterSelect lg:w-40 relative">
                    ...
                    <div className="mt-2">
                        <table className="tableContent hidden md:table w-full text-left border-collapse">
                            <thead className="tHead border-b border-t border-gray-300">
                            ...
                            <tbody className="text-sm">
                                {data?.map((offer, i:number) => (
                                    <tr key={i} className="tData border-b border-gray-300">
                        ...
                        <div className="tDataResponsive block md:hidden space-y-4">
                            {data?.map((offer, i:number) => (
                            <div
                                key={i}
